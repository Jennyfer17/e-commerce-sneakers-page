...
export default function Header({item, removeFromCart}) {
    ...
    return (
        <header className="flex justify-between px-6 py-8 items-center">
            ...
            <div className="flex space-x-4 items-center">
                <Sheet open={open} onOpenChange={setOpen} >
                    <SheetTrigger asChild className="relative">
                            <div>
                                <img src={CartIcon} alt="cart icon" className="w-8 h-8 object-cover"/>
                                {item.quantity > 0 && (
                                <span className="absolute -top-1 right-0 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold">
                                    {item.quantity}
                                </span>
                                                    )}
                            </div>
